...
// Em Compilador.jj, substitua a regra inteira por esta:

// Em Compilador.jj, use esta regra para "atribuicao"
  final public void atribuicao() throws ParseException {
    Token t;
    Simbolo simboloLHS = null; // Inicializa como nulo
    ...
    try {
      ...
      jj_consume_token(SEMICOLON);
        ...
        Token t_recuperacao; // Token para recuperação
        while (true) {
            t_recuperacao = getToken(1);
            if (t_recuperacao.kind == EOF || t_recuperacao.kind == SEMICOLON) {
                 if (t_recuperacao.kind == SEMICOLON) getNextToken();
                 break;
            }
            getNextToken();
        }
  ...
  final public void entrada() throws ParseException {
    ...
    try {
      ...
      t = jj_consume_token(IDENTIFIER);
            ...
            this.indiceConstante = null; // Reseta
          ...
          temIndiceLHS = indice(simboloLHS);
            if (simboloLHS != null) {
                ...
                if (temIndiceLHS) {
                     if (this.indiceConstante != null) {
                        // Otimização para READ (ex: read(v[3]))
                        int indice = Integer.parseInt(this.indiceConstante.image);
                        int endereco = simboloLHS.getBase() + (indice - 1);
                        geradorDeCodigo.gerar("STR", String.valueOf(endereco));
                    } else {
                        // Acesso indireto (ex: read(v[i]))
                        geradorDeCodigo.gerar("STX", "0");
                    }
                } else {
                    geradorDeCodigo.gerar("STR", String.valueOf(simboloLHS.getBase()));
             ...
             if (simboloRHS != null) {
                ...
                if (temIndiceRHS) {
                    if (this.indiceConstante != null) {
                        ...
                        // --- ACESSO INDIRETO (Programa B) ---
                        // A regra 'indice' já gerou o código do ENDEREÇO.
...
/**
 * Regra unificada para tratar acesso a índice de vetor.
 * Esta regra faz a otimização de acesso estático (ex: v[3])
 * e também gera o código para acesso dinâmico (ex: v[i+1]).
 *
 * @param simbolo O símbolo (LHS ou RHS) que está sendo acessado.
 * @return true se um índice (ex: [ ... ]) foi encontrado, false caso contrário.
 */
  ...
  static {
   ...
   private static void jj_la1_init_0() {
      jj_la1_0 = new int[] {0x10000000,0x100,0x10000000,0x10000000,0x0,0x780000,0x0,0x0,0x0,0x0,0xf800000,0x27800,0x27800,0x0,0x10000,0x80000000,0x0,0x0,0x0,0x1f800000,0x1f800000,0x0,};
   }
   private static void jj_la1_init_1() {
      jj_la1_1 = new int[] {0x0,0x0,0x0,0x0,0x20000,0x0,0x8000,0x100000,0x8000,0x20000,0x0,0x0,0x0,0x20000,0x0,0x1f,0x2180,0x1e40,0x20,0x44000,0x44000,0x100000,};
   }
